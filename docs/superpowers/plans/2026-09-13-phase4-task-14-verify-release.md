# Phase 4 Build — Task 14: End-to-end verify, regressions, docs, release
...
### Task 14: End-to-end verify, regressions, docs, release

**Files:**
- Create: `infra/tests/04-admin.sh`
- Modify: `README.md`, `docs/superpowers/specs/2026-09-13-sandcastle-mvp-design.md`, `docs/superpowers/specs/2026-09-13-phase4-admin-control-plane-design.md` (status + implementation findings)

**Interfaces:**
- Consumes: deployed stack (Task 13); secrets `admin-test-token`, `admin-db`; routes from Task 11; test-hook TTL `60s`.

- [ ] **Step 1: Write the suite** — `infra/tests/04-admin.sh`
...
```bash
chmod +x infra/tests/04-admin.sh && bash -n infra/tests/04-admin.sh
git add infra/tests/04-admin.sh
git commit -m "test(admin): add phase 4 end-to-end verify"
```

- [ ] **Step 3: Run it (OWNER runs the make target; the executor may use `$P/vmsync && $P/vmssh 'cd sandcastle && infra/tests/04-admin.sh'`)**

Expected: every line `ok`, then `phase 4 passed`.
On a failure, diagnose from `kubectl -n sandcastle-admin logs deploy/sandcastle-admin`, `kubectl -n sandcastle-egress logs deploy/envoy-egress`, `/tmp/adm-*.log`. Fix in the owning task's files, re-run `make admin-test` (and `make admin-smoke` for xDS changes), rebuild and redeploy (`make admin-image`, then `$P/vmssh 'kubectl -n sandcastle-admin rollout restart deploy/sandcastle-admin'`), and commit each fix as `fix(admin): ...`.

- [ ] **Step 4: Regressions**

Run in the VM (host lock on): `infra/tests/01-isolation-substrate.sh`, `infra/tests/02-dx-baseline.sh`, `infra/tests/03-containment.sh`.
Expected: phase 1 9/9, phase 2 18/18, phase 3 35/35 (the default zone's `example.com` seed keeps the Phase 3 allowlist). Also on the host: **OWNER** `sudo -u "$USER" make verify-host-egress`.

- [ ] **Step 5: Docs**

- **Phase 4 spec:** change `Status:` to `implemented (v0.4.0)`. Append `## Implementation findings — <date>` with one 5-W bullet per bug or deviation found in Tasks 4–14 (who, what, when, where, why), and link the spike results.
- **MVP spec:**
  - Mark Phase 4 "— complete (v0.4.0)".
  - Add an addendum: admin UI on NodePort 30081 via Coder OAuth2.
  - Add another: zones replace `workspace-dns-allow.yaml`.
  - Add another: Envoy is configured only by xDS.
- **README:**
  - Add `admin/` and `platform/admin/` rows to the layout table.
  - Replace the `workspace-dns-allow.yaml` instructions with zone rules in the UI.
  - Add Phase 4 run steps: `make admin-test`, `make admin-smoke`, `make egress-unlock`, `make admin-image`, `make admin`, `make egress-lock`, `make verify-admin`.
  - Status: Phases 1–4 complete (v0.4.0), Phase 5 next.
- **Phase 3 spec, line ~172:** add "(superseded in Phase 4 by admin zones)" after the `workspace-dns-allow.yaml` mention.

```bash
git add README.md docs/superpowers/specs
git commit -m "docs: mark phase 4 admin control plane complete"
```

- [ ] **Step 6: Release gate (OWNER)**

Report the results to the owner: verify output, regression counts, and findings. Wait for approval, then:
```bash
git tag -a v0.4.0 -m "Phase 4: admin control plane"
git push origin main v0.4.0
```
Log the milestone to open-brain (project `c3bcc121-ec02-44f9-8a76-add22f0938a5`), and ask the owner to snapshot: `sudo -u "$USER" infra/vm/sandcastle-vm.sh snapshot phase4-done`.
